...
function guardar_ingreso($id_producto,$fecha,$quantity,$precio){
	global $con;
	$sql="INSERT INTO modelo (id_producto, fecha, cantidad , precio)
	VALUES ('$id_producto', '$fecha', '$quantity', '$precio');";
	$query=mysqli_query($con,$sql);
}
...
function retirar_stock($id_producto,$quantity){
	global $con;
	$update=mysqli_query($con,"update modelo set cantidad=cantidad-'$quantity' where id_producto='$id_producto'");
	if ($update){
			return 1;
	} else {
		return 0;
